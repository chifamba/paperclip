...
description: >
  Create new Agents plugins with the current alpha SDK/runtime. Use when
...
# Create a Agents Plugin

Use this skill when the task is to create, scaffold, or document a Agents plugin.
...
For a plugin that lives outside the Agents repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.paperclip-sdk/`:
...
- is installed into Agents from an absolute local path during development
