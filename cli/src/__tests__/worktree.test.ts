...
describe("worktree helpers", () => {
  ...
  it("rewrites loopback auth URLs to the new port only", () => {
    ...
    expect(rewriteLocalUrlPort("https://paperclip.example", 3110)).toBe(
  ...
  it("builds isolated config and env paths for a worktree", () => {
    ...
    expect(formatShellExports(env)).toContain(
      "export PAPERCLIP_INSTANCE_ID='feature-worktree-support'",
    );
  });
  ...
  it("copies the source local_encrypted secrets key into the seeded worktree instance", () => {
    const tempRoot = fs.realpathSync(
      fs.mkdtempSync(path.join(os.tmpdir(), "paperclip-worktree-secrets-")),
    );
  ...
  it("persists the current agent jwt secret into the worktree env file", async () => {
    const tempRoot = fs.realpathSync(
      fs.mkdtempSync(path.join(os.tmpdir(), "paperclip-worktree-jwt-")),
    );
    ...
    try {
      ...
      expect(envContents).toContain(
        "PAPERCLIP_AGENT_JWT_SECRET=worktree-shared-secret",
      );
  ...
  itEmbeddedPostgres(
    ...
    async () => {
      const tempRoot = fs.realpathSync(
        fs.mkdtempSync(path.join(os.tmpdir(), "paperclip-worktree-auth-seed-")),
      );
      ...
      const sourceDb = await startEmbeddedPostgresTestDatabase(
        "paperclip-worktree-auth-source-",
      );
      ...
      try {
        ...
        const targetConfig = JSON.parse(
          fs.readFileSync(
            path.join(worktreeRoot, ".paperclip", "config.json"),
            "utf8",
          ),
        ) as PaperclipConfig;
        ...
        try {
          ...
          expect(
            seededUsers.some((row) => row.email === "existing@paperclip.ing"),
          ).toBe(true);
        } finally {
          await targetPg.stop();
        }
  ...
  it("avoids ports already claimed by sibling worktree instance configs", async () => {
    const tempRoot = fs.realpathSync(
      fs.mkdtempSync(
        path.join(os.tmpdir(), "paperclip-worktree-claimed-ports-"),
      ),
    );
    const repoRoot = path.join(tempRoot, "repo");
    ...
    try {
      ...
      const config = JSON.parse(
        fs.readFileSync(
          path.join(repoRoot, ".paperclip", "config.json"),
          "utf8",
        ),
      );
  ...
  it("defaults the seed source config to the current repo-local Paperclip config", () => {
    const tempRoot = fs.realpathSync(
      fs.mkdtempSync(
        path.join(os.tmpdir(), "paperclip-worktree-source-config-"),
      ),
    );
    const repoRoot = path.join(tempRoot, "repo");
  ...
  it("preserves the source config path across worktree:make cwd changes", () => {
    const tempRoot = fs.realpathSync(
      fs.mkdtempSync(
        path.join(os.tmpdir(), "paperclip-worktree-source-override-"),
      ),
    );
    const sourceConfigPath = path.join(tempRoot, "source", "config.json");
  ...
  it("derives worktree reseed target paths from the adjacent env file", () => {
    const tempRoot = fs.realpathSync(
      fs.mkdtempSync(
        path.join(os.tmpdir(), "paperclip-worktree-reseed-target-"),
      ),
    );
  ...
  it("rejects reseed targets without worktree env metadata", () => {
    const tempRoot = fs.realpathSync(
      fs.mkdtempSync(
        path.join(os.tmpdir(), "paperclip-worktree-reseed-target-missing-"),
      ),
    );
    const worktreeRoot = path.join(tempRoot, "repo");
    const configPath = path.join(worktreeRoot, ".paperclip", "config.json");
    ...
    try {
      ...
      fs.writeFileSync(configPath, JSON.stringify(buildSourceConfig()), "utf8");
      fs.writeFileSync(
        path.join(worktreeRoot, ".paperclip", ".env"),
        "",
        "utf8",
      );
  ...
  it("reseed preserves the current worktree ports, instance id, and branding", async () => {
    const tempRoot = fs.realpathSync(
      fs.mkdtempSync(path.join(os.tmpdir(), "paperclip-worktree-reseed-")),
    );
    const repoRoot = path.join(tempRoot, "repo");
  ...
  it("restores the current worktree config and instance data if reseed fails", async () => {
    const tempRoot = fs.realpathSync(
      fs.mkdtempSync(
        path.join(os.tmpdir(), "paperclip-worktree-reseed-rollback-"),
      ),
    );
    const repoRoot = path.join(tempRoot, "repo");
    const sourceRoot = path.join(tempRoot, "source");
  ...
  it.skip("copies shared git hooks into a linked worktree git dir", () => {
    const tempRoot = fs.realpathSync(
      fs.mkdtempSync(path.join(os.tmpdir(), "paperclip-worktree-hooks-")),
    );
    const repoRoot = path.join(tempRoot, "repo");
  ...
  it("creates and initializes a worktree from the top-level worktree:make command", async () => {
    const tempRoot = fs.realpathSync(
      fs.mkdtempSync(path.join(os.tmpdir(), "paperclip-worktree-make-")),
    );
    const repoRoot = path.join(tempRoot, "repo");
    ...
    try {
      ...
      expect(
        fs.existsSync(path.join(worktreePath, ".paperclip", "config.json")),
      ).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".paperclip", ".env"))).toBe(
        true,
      );
    } finally {
      process.chdir(originalCwd);
  ...
  it("no-ops on the primary checkout unless --branch is provided", async () => {
    const tempRoot = fs.realpathSync(
      fs.mkdtempSync(
        path.join(os.tmpdir(), "paperclip-worktree-repair-primary-"),
      ),
    );
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      expect(
        fs.existsSync(path.join(repoRoot, ".paperclip", "config.json")),
      ).toBe(false);
      expect(
        fs.existsSync(path.join(repoRoot, ".paperclip", "worktrees")),
      ).toBe(false);
    } finally {
      process.chdir(originalCwd);
      fs.rmSync(tempRoot, { recursive: true, force: true });
  ...
  it("repairs the current linked worktree when Paperclip metadata is missing", async () => {
    const tempRoot = fs.realpathSync(
      fs.mkdtempSync(
        path.join(os.tmpdir(), "paperclip-worktree-repair-current-"),
      ),
    );
    const repoRoot = path.join(tempRoot, "repo");
  ...
  it("creates and repairs a missing branch worktree when --branch is provided", async () => {
    const tempRoot = fs.realpathSync(
      fs.mkdtempSync(
        path.join(os.tmpdir(), "paperclip-worktree-repair-branch-"),
      ),
    );
    const repoRoot = path.join(tempRoot, "repo");
    const sourceConfigPath = path.join(tempRoot, "source-config.json");
    ...
    try {
      ...
      expect(
        fs.existsSync(
          path.join(expectedWorktreePath, ".paperclip", "config.json"),
        ),
      ).toBe(true);
      expect(
...
describe("worktreeCleanupCommand", () => {
  it("cleans up a completely populated worktree successfully", async () => {
    const tempRoot = fs.realpathSync(
      fs.mkdtempSync(
        path.join(os.tmpdir(), "paperclip-worktree-cleanup-happy-"),
      ),
    );
    const repoRoot = path.join(tempRoot, "repo");
    const fakeHome = path.join(tempRoot, "home");
    ...
    try {
      ...
      execFileSync("git", ["branch", name], { cwd: repoRoot, stdio: "ignore" });
      execFileSync("git", ["worktree", "add", targetPath, name], {
        cwd: repoRoot,
        stdio: "ignore",
      });

      const instanceId = sanitizeWorktreeInstanceId(name);
      const instanceRoot = path.join(worktreeHome, "instances", instanceId);
      fs.mkdirSync(instanceRoot, { recursive: true });
      fs.writeFileSync(path.join(instanceRoot, "test.txt"), "data");

      process.chdir(repoRoot);

      await worktreeCleanupCommand(name, {
        instance: name,
        home: worktreeHome,
      });
      ...
      const expectedInstanceRoot = path.resolve(
        worktreeHome,
        "instances",
        sanitizeWorktreeInstanceId(name),
      );
      expect(fs.existsSync(expectedInstanceRoot)).toBe(false);
  ...
  it("returns gracefully when nothing exists to clean up", async () => {
    const tempRoot = fs.realpathSync(
      fs.mkdtempSync(
        path.join(os.tmpdir(), "paperclip-worktree-cleanup-noop-"),
      ),
    );
    const repoRoot = path.join(tempRoot, "repo");
    const fakeHome = path.join(tempRoot, "home");
  ...
  it("throws an error when the worktree has uncommitted changes, without --force", async () => {
    const tempRoot = fs.realpathSync(
      fs.mkdtempSync(
        path.join(os.tmpdir(), "paperclip-worktree-cleanup-uncommitted-err-"),
      ),
    );
    const repoRoot = path.join(tempRoot, "repo");
    const fakeHome = path.join(tempRoot, "home");
    ...
    try {
      ...
      const name = "cleanup-uncommitted-test";
      const targetPath = resolveWorktreeMakeTargetPath(name);
      ...
      try {
        ...
        expect(e.message).toMatch(/Safety checks failed/);
      }
    } finally {
      process.chdir(originalCwd);
      homedirSpy.mockRestore();
  ...
  it("successfully cleans up despite safety warnings when --force is provided", async () => {
    const tempRoot = fs.realpathSync(
      fs.mkdtempSync(
        path.join(os.tmpdir(), "paperclip-worktree-cleanup-force-"),
      ),
    );
    const repoRoot = path.join(tempRoot, "repo");
    const fakeHome = path.join(tempRoot, "home");
    ...
    try {
      ...
      // Add a unique commit to the branch
      fs.writeFileSync(path.join(targetPath, "unique.txt"), "data", "utf8");
      execFileSync("git", ["add", "unique.txt"], {
        cwd: targetPath,
        stdio: "ignore",
      });
      execFileSync("git", ["commit", "-m", "Unique commit"], {
        cwd: targetPath,
        stdio: "ignore",
      });

      // Add an uncommitted change
      fs.writeFileSync(path.join(targetPath, "unique.txt"), "modified", "utf8");
